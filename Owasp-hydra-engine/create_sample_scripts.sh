#!/usr/bin/env bash
# create_sample_scripts.sh
# Run this from the project root: e.g.
# cd ~/Desktop/"New Folder"/owasp-hydra-engine
# chmod +x create_sample_scripts.sh
# ./create_sample_scripts.sh

set -euo pipefail

ROOT_DIR="$(pwd)"
FOLDERS=(A01 A02 A03 A04 A05 A06 A07 A08 A09 A10)

echo "Creating folders and demo scripts under: $ROOT_DIR"

for f in "${FOLDERS[@]}"; do
  mkdir -p "$ROOT_DIR/$f"
  cat > "$ROOT_DIR/$f/demo.py" <<'PY'
#!/usr/bin/env python3
"""
...
PY
  ...
  chmod +x "$ROOT_DIR/$f/demo.py"

  # add a tiny README in each folder
  cat > "$ROOT_DIR/$f/README.txt" <<EOF
This folder: $f
...
echo "All demo scripts created in: ${FOLDERS[*]}"
echo
echo "Quick test (run one):"
echo "  python3 A01/demo.py"
echo "Or run with a target:"
echo "  python3 A01/demo.py --target http://test.local"
